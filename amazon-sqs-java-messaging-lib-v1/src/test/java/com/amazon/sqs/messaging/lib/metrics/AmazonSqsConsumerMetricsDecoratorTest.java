...
class AmazonSqsConsumerMetricsDecoratorTest {
  ...
  @Mock
  private AmazonSqsConsumer<SendMessageBatchRequest, SendMessageBatchResult> delegate;

  @Mock
  private QueueProperty queueProperty;

  private AmazonSqsConsumerMetricsDecorator decorator;

  @BeforeEach
  void setUp() {
    when(queueProperty.getQueueUrl()).thenReturn(QUEUE_URL);

    decorator = new AmazonSqsConsumerMetricsDecorator(delegate, queueProperty, meterRegistry);
  }

  @Test
  void testConstructorRegistersPublishAttemptsCounter() {
    final Counter counter = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_ATTEMPTS)
      .tag("queue", QUEUE_URL)
      .counter();
  ...
  void testConstructorRegistersPublishSuccessCounter() {
    final Counter counter = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_SUCCESS)
      .tag("queue", QUEUE_URL)
      .counter();
  ...
  void testConstructorRegistersPublishDurationTimer() {
    final Timer timer = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_DURATION)
      .tag("queue", QUEUE_URL)
      .timer();
  ...
  void testConstructorRegistersPublishBatchSizeSummary() {
    final DistributionSummary summary = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_BATCH_SIZE)
      .tag("queue", QUEUE_URL)
      .summary();
  ...
  void testConstructorRegistersInflightGauge() {
    final Gauge gauge = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_INFLIGHT)
      .tag("queue", QUEUE_URL)
      .gauge();
  ...
  void testConstructorWithNullMeterRegistryDoesNotThrow() {
    final AmazonSqsConsumerMetricsDecorator nullRegistryDecorator = new AmazonSqsConsumerMetricsDecorator(delegate, queueProperty, null);
  ...
  void testPublishIncrementsAttemptsCounter() {
    final SendMessageBatchRequest request = buildRequest(3);
    when(delegate.publish(request)).thenReturn(new SendMessageBatchResult());

    decorator.publish(request);

    final double count = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_ATTEMPTS)
      .tag("queue", QUEUE_URL)
      .counter()
      .count();
  ...
  void testPublishRecordsBatchSize() {
    final SendMessageBatchRequest request = buildRequest(5);
    when(delegate.publish(request)).thenReturn(new SendMessageBatchResult());

    decorator.publish(request);

    final double mean = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_BATCH_SIZE)
      .tag("queue", QUEUE_URL)
      .summary()
      .mean();
  ...
  void testPublishRecordsTimer() {
    final SendMessageBatchRequest request = buildRequest(2);
    when(delegate.publish(request)).thenReturn(new SendMessageBatchResult());

    decorator.publish(request);

    final long timerCount = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_DURATION)
      .tag("queue", QUEUE_URL)
      .timer()
      .count();
  ...
  void testPublishInflightGaugeIsZeroAfterCompletion() {
    final SendMessageBatchRequest request = buildRequest(1);
    when(delegate.publish(request)).thenReturn(new SendMessageBatchResult());

    decorator.publish(request);

    final double inflight = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_INFLIGHT)
      .tag("queue", QUEUE_URL)
      .gauge()
      .value();
  ...
  void testPublishDelegatesCallToDelegate() {
    final SendMessageBatchRequest request = buildRequest(2);
    final SendMessageBatchResult expectedResult = new SendMessageBatchResult();
    when(delegate.publish(request)).thenReturn(expectedResult);

    final SendMessageBatchResult result = decorator.publish(request);
  ...
  void testHandleResponseIncrementsSuccessCounter() {
    final SendMessageBatchResult result = buildResult(3, 0);

    decorator.handleResponse(result);

    final double count = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_SUCCESS)
      .tag("queue", QUEUE_URL)
      .counter()
      .count();
  ...
  void testHandleResponseDoesNotIncrementSuccessCounterWhenZeroSuccessful() {
    final SendMessageBatchResult result = buildResult(0, 2);
  ...
  void testHandleResponseIncrementsFailureCounterForEachFailedEntry() {
    ...
    final double count1 = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_FAILURE)
      .tag("queue", QUEUE_URL)
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_CODE, "InvalidParameter")
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_TYPE, AbstractAmazonSqsConsumerMetricsDecorator.ERROR_TYPE_AMAZON)
      .counter()
      .count();

    final double count2 = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_FAILURE)
      .tag("queue", QUEUE_URL)
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_CODE, "AuthorizationError")
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_TYPE, AbstractAmazonSqsConsumerMetricsDecorator.ERROR_TYPE_AMAZON)
      .counter()
      .count();
  ...
  void testHandleResponseDelegatesCallToDelegate() {
    final SendMessageBatchResult result = buildResult(1, 0);
  ...
  void testHandleResponseWithMixedSuccessAndFailure() {
    final SendMessageBatchResult result = buildResult(2, 1);
    result.getFailed().get(0).setCode("Throttling");

    decorator.handleResponse(result);

    final double successCount = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_SUCCESS)
      .tag("queue", QUEUE_URL)
      .counter()
      .count();

    final double failureCount = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_FAILURE)
      .tag("queue", QUEUE_URL)
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_CODE, "Throttling")
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_TYPE, AbstractAmazonSqsConsumerMetricsDecorator.ERROR_TYPE_AMAZON)
      .counter()
      .count();
  ...
  void testHandleErrorWithAmazonServiceExceptionIncrementsFailureCounter() {
    ...
    final double count = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_FAILURE)
      .tag("queue", QUEUE_URL)
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_CODE, "ServiceUnavailable")
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_TYPE, AbstractAmazonSqsConsumerMetricsDecorator.ERROR_TYPE_AMAZON)
      .counter()
      .count();
  ...
  void testHandleErrorWithUnknownExceptionIncrementsFailureCounterWithCode000() {
    ...
    final double count = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_FAILURE)
      .tag("queue", QUEUE_URL)
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_CODE, "000")
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_TYPE, AbstractAmazonSqsConsumerMetricsDecorator.ERROR_TYPE_OTHER)
      .counter()
      .count();
  ...
  void testHandleErrorWithAmazonServiceExceptionUsesAmazonErrorType() {
    ...
    final Counter counter = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_FAILURE)
      .tag("queue", QUEUE_URL)
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_TYPE, AbstractAmazonSqsConsumerMetricsDecorator.ERROR_TYPE_AMAZON)
      .counter();
  ...
  void testHandleErrorCountsAllEntriesInBatch() {
    final SendMessageBatchRequest request = buildRequest(10);
  ...
  void testFullSuccessfulPublishFlowUpdatesAllMetrics() {
    final SendMessageBatchRequest request = buildRequest(4);
    final SendMessageBatchResult batchResult = buildResult(4, 0);
    when(delegate.publish(request)).thenReturn(batchResult);

    final SendMessageBatchResult result = decorator.publish(request);
    decorator.handleResponse(result);

    final double attempts = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_ATTEMPTS)
      .tag("queue", QUEUE_URL)
      .counter()
      .count();

    final double success = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_SUCCESS)
      .tag("queue", QUEUE_URL)
      .counter()
      .count();

    final double batchMean = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_BATCH_SIZE)
      .tag("queue", QUEUE_URL)
      .summary()
      .mean();
  ...
  void testFullErrorFlowUpdatesFailureMetrics() {
    ...
    final double failures = meterRegistry.find(AbstractAmazonSqsConsumerMetricsDecorator.METRIC_PUBLISH_FAILURE)
      .tag("queue", QUEUE_URL)
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_CODE, "RequestExpired")
      .tag(AbstractAmazonSqsConsumerMetricsDecorator.TAG_ERROR_TYPE, AbstractAmazonSqsConsumerMetricsDecorator.ERROR_TYPE_AMAZON)
      .counter().count();
  ...
  private SendMessageBatchRequest buildRequest(final int size) {
    final List<SendMessageBatchRequestEntry> entries = new ArrayList<>();

    IntStream.range(0, size).forEach(i -> {
      entries.add(new SendMessageBatchRequestEntry().withId("id-" + i).withMessageBody("msg-" + i));
    });

    return new SendMessageBatchRequest().withEntries(entries);
  }

  private SendMessageBatchResult buildResult(final int successCount, final int failureCount) {
    final List<SendMessageBatchResultEntry> successful = new ArrayList<>();

    IntStream.range(0, successCount).forEach(i -> {
      successful.add(new SendMessageBatchResultEntry().withId("id-" + i).withMessageId("msg-id-" + i));
    });
    ...
    return new SendMessageBatchResult().withSuccessful(successful).withFailed(failed);
  }
