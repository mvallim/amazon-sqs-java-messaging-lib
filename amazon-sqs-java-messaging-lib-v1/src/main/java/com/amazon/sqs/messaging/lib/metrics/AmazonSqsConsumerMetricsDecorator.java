...
// @formatter:off
/**
 * AWS SDK v1 metrics decorator for {@link AmazonSqsConsumer}. Records publish attempt/success/failure
 ...
 */
public class AmazonSqsConsumerMetricsDecorator extends AbstractAmazonSqsConsumerMetricsDecorator<SendMessageBatchRequest, SendMessageBatchResult> {

  private static final Logger LOGGER = LoggerFactory.getLogger(AmazonSqsConsumerMetricsDecorator.class);

  /**
   * Creates a new v1 SQS consumer metrics decorator.
   *
  ...
  public AmazonSqsConsumerMetricsDecorator(
      final AmazonSqsConsumer<SendMessageBatchRequest, SendMessageBatchResult> delegate,
  ...
  @Override
  @SneakyThrows
  public SendMessageBatchResult publish(final SendMessageBatchRequest publishBatchRequest) {
    publishAttemptsCounter.increment();
    batchSizeSummary.record(publishBatchRequest.getEntries().size());
  ...
  public void handleError(final SendMessageBatchRequest publishBatchRequest, final Throwable throwable) {
    ...
    final int failedEntries = publishBatchRequest.getEntries().size();
