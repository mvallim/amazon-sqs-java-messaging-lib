...
class AmazonSqsConsumerMetricsDecoratorTest {
  ...
  @Mock
  private AmazonSqsConsumer<SendMessageBatchRequest, SendMessageBatchResponse> delegate;
...
private QueueProperty queueProperty;
  ...
  void testPublishIncrementsAttemptsCounter() {
    ...
    when(delegate.publish(request)).thenReturn(SendMessageBatchResponse.builder().build());
  ...
  void testPublishDelegatesCallToDelegate() {
    final SendMessageBatchRequest request = buildRequest(2);
    final SendMessageBatchResponse expectedResult = SendMessageBatchResponse.builder().build();
    when(delegate.publish(request)).thenReturn(expectedResult);

    final SendMessageBatchResponse result = decorator.publish(request);
  ...
  void testHandleResponseIncrementsSuccessCounter() {
    final SendMessageBatchResponse result = buildResult(3, 0);
  ...
  void testHandleResponseDoesNotIncrementSuccessCounterWhenZeroSuccessful() {
    final SendMessageBatchResponse result = buildResult(0, 2);
  ...
  void testHandleResponseDelegatesCallToDelegate() {
    final SendMessageBatchResponse result = buildResult(1, 0);
  ...
  void testHandleResponseWithMixedSuccessAndFailure() throws IllegalAccessException {
    final SendMessageBatchResponse result = buildResult(2, 1);
  ...
  void testFullSuccessfulPublishFlowUpdatesAllMetrics() {
    ...
    final SendMessageBatchResponse batchResult = buildResult(4, 0);
  ...
  private SendMessageBatchRequest buildRequest(final int size) {
    ...
    IntStream.range(0, size).forEach(i -> {
      entries.add(SendMessageBatchRequestEntry.builder().id("id-" + i).messageBody("msg-" + i).build());
    });

    return SendMessageBatchRequest.builder().entries(entries).build();
  }

  private SendMessageBatchResponse buildResult(final int successCount, final int failureCount) {
    final List<SendMessageBatchResultEntry> successful = new ArrayList<>();

    IntStream.range(0, successCount).forEach(i -> {
      successful.add(SendMessageBatchResultEntry.builder().id("id-" + i).messageId("msg-id-" + i).build());
    });
