...
Shared abstract module providing the core framework for batched Amazon SQS sending. All SDK-specific implementations (`-v1`, `-v2`) depend on this module.
...
| Class                                            | Description                                                                                                                                                                                        |
|--------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `AbstractAmazonSqsTemplate<R,O,E>`               | High-level template. Composes a producer + consumer. Fluent builder with defaults: `ConcurrentHashMap` for pending requests, `RingBufferBlockingQueue`, identity decorator, `SimpleMeterRegistry`. |
| `AbstractAmazonSqsProducer<E>`                   | Enqueues `RequestEntry` into a `BlockingQueue`. Tracks lifecycle state (`RUNNIG`/`SHUTDOWN`).                                                                                                      |
| `AbstractAmazonSqsConsumer<C,R,O,E>`             | Periodic batch drain loop. Polls queue, assembles batches respecting 1024KB limit, sends via SDK client, dispatches success/failure callbacks.                                                     |
| `AmazonSqsConsumer<R,O>`                         | Public interface: `publish()`, `handleError()`, `handleResponse()`, `shutdown()`, `await()`.                                                                                                       |
| `AmazonSqsProducer<E>`                           | Public interface: `send()`, `shutdown()`.                                                                                                                                                          |
...
| Class                         | Description                                                                                                                                                                                    |
|-------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `AmazonSqsThreadPoolExecutor` | `ThreadPoolExecutor` with zero core threads, `SynchronousQueue`, 30-second blocking rejection policy.                                                                                          |
...
| Class                                            | Description                                                                                                                                           |
|--------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| `AbstractAmazonSqsConsumerMetricsDecorator<I,O>` | Abstract Micrometer decorator. Tracks `sqs.publish.attempts`, `sqs.publish.success`, `sqs.publish.failure`, duration, batch size, and inflight gauge. |
...
| Class                        | Description                                                                                                                      |
|------------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| `RequestEntry<T>`            | Single message: payload, headers, FIFO group/deduplication IDs. Auto-generates `createTime` (nanoTime) and `id` (UUID).          |
| `QueueProperty`              | Queue config: `fifo`, `maximumPoolSize`, `queueUrl`, `linger`, `maxBatchSize`.                                                   |
...
| `MaximumAllowedMessageException` | Thrown when a single serialized message exceeds 1024KB. |
| `JsonProcessingException`        | Thrown when a request payload fails to serialize to JSON. |

## Dependencies

Managed by parent POM `com.github.mvallim:amazon-sqs-java-messaging-lib:1.3.2`:
