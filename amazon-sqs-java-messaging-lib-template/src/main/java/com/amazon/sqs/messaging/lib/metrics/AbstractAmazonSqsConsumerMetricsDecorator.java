 ...
 */
abstract class AbstractAmazonSqsConsumerMetricsDecorator<I, O> implements AmazonSqsConsumer<I, O> {

  /** Base tag prefix for all SQS metrics. */
  private static final String TAG_SQS = "sqs";

  /** Metric name for total publish attempts. */
  protected static final String METRIC_PUBLISH_ATTEMPTS = TAG_SQS.concat(".publish.attempts");

  /** Metric name for successful publish operations. */
  protected static final String METRIC_PUBLISH_SUCCESS = TAG_SQS.concat(".publish.success");

  /** Metric name for failed publish operations. */
  protected static final String METRIC_PUBLISH_FAILURE = TAG_SQS.concat(".publish.failure");

  /** Metric name for publish latency duration. */
  protected static final String METRIC_PUBLISH_DURATION = TAG_SQS.concat(".publish.duration");

  /** Metric name for publish batch size distribution. */
  protected static final String METRIC_PUBLISH_BATCH_SIZE = TAG_SQS.concat(".publish.batch.size");

  /** Metric name for inflight publish count. */
  protected static final String METRIC_PUBLISH_INFLIGHT = TAG_SQS.concat(".publish.inflight");
  ...
  /** Metric tags identifying the SQS queue. */
  ...
  /** The decorated {@link AmazonSqsConsumer} delegate. */
  protected final AmazonSqsConsumer<I, O> delegate;

  /**
   * Creates a new metrics decorator.
   *
   * @param delegate      the consumer to decorate
   * @param queueProperty the queue configuration (used for queue tags)
   * @param meterRegistry the Micrometer meter registry (may be null)
   */
  AbstractAmazonSqsConsumerMetricsDecorator(
     final AmazonSqsConsumer<I, O> delegate,
    ...
    tags = Tags.of("queue", queueProperty.getQueueUrl());

    publishAttemptsCounter = Counter.builder(METRIC_PUBLISH_ATTEMPTS)
      .tags(tags)
      .description("Total number of SQS PublishBatch calls attempted")
      .register(compositeMeterRegistry);

    successCounter = Counter.builder(METRIC_PUBLISH_SUCCESS)
      .tags(tags)
      .description("Individual SQS messages acknowledged as successful")
      .register(compositeMeterRegistry);

    publishTimer = Timer.builder(METRIC_PUBLISH_DURATION)
      .tags(tags)
      .description("End-to-end latency of SQS PublishBatch calls")
      .publishPercentiles(0.5, 0.95, 0.99)
      .register(compositeMeterRegistry);

    batchSizeSummary = DistributionSummary.builder(METRIC_PUBLISH_BATCH_SIZE)
      .tags(tags)
      .description("Number of entries per SQS PublishBatch request")
      .register(compositeMeterRegistry);
